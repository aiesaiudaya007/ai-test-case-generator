...
- **Removes manual test case authoring for standard stories.** Demonstrated
  end-to-end on a sample story: 6 acceptance criteria expanded automatically
  into 12 test scenarios (positive + negative paths) and 12 detailed,
  executable test cases — work that would otherwise be a manual QA drafting
  pass on every story.
- **Closes the "someone has to remember to write tests" gap.** The webhook
  integration triggers generation the moment a story reaches "Ready for QA"
  in Jira, so coverage isn't dependent on a QA engineer's bandwidth or memory
  to kick it off.
- **Improves traceability without extra documentation effort.** Every
  generated scenario links back to the specific acceptance criterion it
  covers, and every test case links back to its scenario — giving direct
  AC → scenario → test case lineage for audits or coverage reviews.
- **Addresses the reliability gap that stalls most AI test-gen pilots.**
  Output is enforced via schema-constrained tool-calling (not free-text
  parsing), with retry/backoff on API calls — the two things that most
  commonly make "AI-generated tests" too unreliable to trust in practice.
- **Drops straight into existing QA tooling.** Output exports to an Excel
  format structured for direct import into TestRail/Zephyr/Xray, removing
  the manual reformatting step between "AI produced something" and "it's
  usable by the team."
- **Lowers the cost of piloting AI in QA.** An offline mock mode runs the
  full pipeline without API credentials or cost, so the approach can be
  evaluated and demoed before committing to production API usage.

*(These reflect the pipeline's demonstrated capability and design intent —
this is a portfolio/reference implementation, not yet metrics from a live
